...
e = 0.3
i = 10.0 # [deg]
omega = 180. # [deg] # omega_1 = 180, so omega_2 = 0
Omega = 0. # [deg]
...
dates = np.linspace(T0, T0 + P, num=300)
...
ax.plot(alpha_dec_ABs[:,1], alpha_dec_ABs[:,0], color="C1")
ax.plot(0,0, "*k", ms=10)
ax.set_xlabel(r"$\Delta \alpha$ mas")
ax.set_ylabel(r"$\Delta \delta$ mas")
# Flip the X axis for RA.
ax.set_xlim(0.120, -0.120) # reverse to put RA increasing to left (east)
ax.set_ylim(-0.120, 0.120)
fig.savefig(plot_dir + "orbit_B_rel_A.png")

# Now plot A and B together
fig, ax = plt.subplots(nrows=1, figsize=(5,5))
ax.plot(alpha_dec_As[:,1], alpha_dec_As[:,0], color="C0")
ax.plot(alpha_dec_Bs[:,1], alpha_dec_Bs[:,0], color="C1")
...
# Flip the X axis for RA.
fig.savefig(plot_dir +"orbit_AB.png")


# Plot the orbits in the plane
fig, ax = plt.subplots(nrows=1, figsize=(5,5))

ax.plot(xy_ABs[:,0], xy_ABs[:,1], ".", color="C1")
# print(xy_ABs[:,0], ".")
# print(xy_ABs[:,1], ".")
ax.plot(0,0, "*k", ms=10)
...
ax.plot(xy_As[:,0], xy_As[:,1], ".", color="C0")
ax.plot(xy_Bs[:,0], xy_Bs[:,1], ".", color="C1")
